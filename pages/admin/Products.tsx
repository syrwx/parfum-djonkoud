...
const ProductsManager: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingProduct.id) {
      success = await updateProduct(editingProduct.id, editingProduct);
    } else {
      const newId = Date.now().toString();
      success = await addProduct({ ...editingProduct, id: newId, notes: editingProduct.notes || [] } as Product);
    }
          ...
          <div className="bg-neutral-900 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-amber-900/30 p-6 relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="pt-4 border-t border-neutral-800 flex justify-end gap-4">
                <button type="button" onClick={closeModal} className="px-6 py-3 text-neutral-400 hover:text-white" disabled={isSubmitting}>Annuler</button>
                <Button type="submit" disabled={isSubmitting}>
                  {isSubmitting ? <span className="flex items-center gap-2"><Loader2 className="animate-spin" size={18}/> Envoi...</span> : 'Enregistrer'}
                </Button>
              </div>
            </form>
          </div>
