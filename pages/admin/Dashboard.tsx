...
import { DollarSign, ShoppingBag, Package, AlertTriangle, Database, CheckCircle2, XCircle } from 'lucide-react';
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-end border-b border-amber-900/30 pb-6">
        ...
        <div className={`flex items-center gap-2 px-4 py-2 rounded-full border ${
          ...
          {dbStatus === 'connected' && <CheckCircle2 size={18} />}
          {dbStatus === 'error' && <XCircle size={18} />}
          {dbStatus === 'checking' && <Database size={18} className="animate-pulse" />}
          
          <span className="text-xs font-bold uppercase tracking-wider">
            {dbStatus === 'connected' ? 'Système Synchronisé' : 
             dbStatus === 'error' ? 'Erreur Connexion DB' : 
             'Vérification...'}
          </span>
        </div>
      </div>
